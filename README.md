...
A JSON oracle (`tests/data/cross_oracle.json`) holds 65 reference `(distribution, kwargs) → (mean, var, ppf)` tuples generated by the Julia package via `scripts/build_cross_oracle.jl`. The test `tests/test_cross_julia.py` runs each case through Python's `make_dist` and asserts agreement to `rel_tol=1e-5`. Regenerate after edits with:

```
julia --project=../DistributionsFactories.jl scripts/build_cross_oracle.jl
```

(The pure-Python test suite is otherwise self-contained — round-trip tests, feasibility-boundary tests, and per-distribution behaviour tests don't need Julia at test time.)
